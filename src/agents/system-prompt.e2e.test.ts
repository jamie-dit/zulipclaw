...
describe("buildAgentSystemPrompt", () => {
  ...
  it("marks system message blocks as internal and not user-visible", async () => {
    const prompt = await buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
    });
  ...
  it("guides subagent workflows to avoid polling loops", async () => {
    const prompt = await buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
    });
  ...
  it("ignores context files with missing or blank paths", async () => {
    const prompt = await buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      contextFiles: [
  ...
  it("includes inline button style guidance when runtime supports inline buttons", async () => {
    const prompt = await buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      toolNames: ["message"],
      runtimeInfo: {
